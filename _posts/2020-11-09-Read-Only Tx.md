...
```
 ...
 基本的Basic PORT设计中，数据保存在一个多版本的store中，而与server交互的client lib会维护version clock的信息。注意这里的逻辑只支持了单行写事务。Client在读取对于版本的时候，server端如果有对于的版本，则直接返回，如果没有，则查找更早的最佳的版本的数据，同时更新这里数据的max r vs的信息，来保证后面的写入请求的vs更大。这种操作在这里称之为Promotion。在接受到一个写请求的时候，这个请求到的vs比目前的max w vs更小的时候，这个写入请求就被忽略了，相当于被后面的写请求覆盖了。后面就是更加max r vs选择实际写入的版本，并更新VersionClock.maxVS的信息然后返回VersionClock.maxVS。这里Write omission在Paper中证明了会是安全的。
