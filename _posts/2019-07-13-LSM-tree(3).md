...
* Opportunistically allocating I/O bandwidth，使用另外的一个线程，来监控系统的负载情况。加入系统可以提供的带宽是T，客户端使用了的带宽是C，则系统会将剩下的带宽I = T −C−ε分配给内部操作，其中ε是一个留下的buffer，避免用得“太紧”。通过使用一个rate limiter来限制IO带宽的使用。为了避免频繁调整带宽限制，这里只有在新的测量值和旧的值相差大于一个阈值的时候才会调整。
